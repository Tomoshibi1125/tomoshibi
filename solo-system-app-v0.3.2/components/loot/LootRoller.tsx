...
export function LootRoller({ level }: { level: number }) {
  ...
  const generateLoot = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Loot generation failed.");
      }
  ...
  return (
    <div className="space-y-2 rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
      ...
      <p className="text-[11px] text-slate-400">
        Your DM says &quot;roll for loot&quot;. Roll a d100 here or use physical dice and
        type it in; the System will generate an item scaled to your level.
      </p>
      ...
      <button onClick={generateLoot} disabled={loading}>
