...
const LABELS: (keyof Props["stats"])[] = ["STR", "AGI", "VIT", "INT", "SENSE"];
  ...
  const spendPoint = async (stat: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "Failed to spend stat point.");
      }
      router.refresh();
    } catch (e: any) {
  ...
  return (
    <div className="mt-3 rounded-md border border-slate-800 bg-slate-950/70 p-2 text-[11px]">
      ...
      <p className="mt-1 text-[10px] text-slate-500">
        Each level grants +3 points you can invest into STR, AGI, VIT, INT, or SENSE, mirroring
